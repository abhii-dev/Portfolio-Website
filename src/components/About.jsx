...
const About = () => {
  ...
  const { ref: introRef, inView: introInView } = useInView({ triggerOnce: true, threshold: 0.5 });
  const { ref: imgRef, inView: imgInView } = useInView({ triggerOnce: true, threshold: 0.2 });
  const { ref: paraRef, inView: paraInView } = useInView({ triggerOnce: true, threshold: 0.01 });
  const { ref: expRef, inView: expInView } = useInView({ triggerOnce: true, threshold: 0.01 });
  const { ref: projRef, inView: projInView } = useInView({ triggerOnce: true, threshold: 0.01 });
        ...
        <div className='flex flex-col md:flex-row justify-center items-center'>
          ...
          <div className="mb-8 md:mb-0 flex justify-center">
            <motion.div
              ...
              animate={imgInView ? { opacity: 1, x: 100 } : {}}
          ...
          <motion.p
            ref={paraRef}
            initial={{ opacity: 0, x: 100 }}
            animate={paraInView ? { opacity: 1, x: 0 } : {}}
            transition={{ delay: 0.6, duration: 0.5 }}
            ...
            a keen interest in building efficient, scalable web applications. Beyond coding, I enjoy immersing myself in video games and movies,
            which fuel my creativity and problem-solving mindset. Always eager to learn new technologies and take on challenging projects that push me to grow as a developer.
          </motion.p>
